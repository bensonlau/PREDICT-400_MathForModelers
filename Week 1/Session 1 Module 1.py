...
pi=3.14159
volume=((4/3)*pi*r**3)
print(volume) #answer in answer sheet rounded to one decimal place: print(round(volume,1))
...
cost=price*discount*x+(3+0.75*(x-1))
...
start_time_seconds=6*60+52
time_spent_running=2*(8+15/60.0)+3*(7+12/60.0) 
total_time_seconds=start_time_seconds+time_spent_running
hour=int(total_time_seconds/60)
minutes=int(round((total_time_seconds%60),0))
print(str(hour)+":"+str(minutes))

###Utilizing datetime and time libraries as directed on 
##https://canvas.northwestern.edu/courses/62050/discussion_topics/356154

#Set Format for time to be printed out
import datetime

#year, month and day are required - chose today's date
start_time = datetime.datetime(year=2017, month=6, day=25,  hour=6, minute=52)

easy_time=datetime.timedelta(minutes=8, seconds=15)
tempo_time=datetime.timedelta(minutes=7, seconds=12)
easy_miles=2
tempo_miles=3

easy_time_total=easy_time*easy_miles
tempo_time_total=tempo_time*tempo_miles

running_time=easy_time_total+tempo_time_total

final_time=start_time+running_time

format_final_time=final_time.strftime('%I:%M') #https://docs.python.org/3.4/library/datetime.html#strftime-and-strptime-behavior

print(format_final_time)